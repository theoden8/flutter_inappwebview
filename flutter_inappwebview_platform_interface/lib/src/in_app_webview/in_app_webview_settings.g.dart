...
class InAppWebViewSettings {
  ...
  // [WebSpace fork patch] UA-CH metadata override.
  ///User-Agent Client Hints metadata. Controls both `Sec-CH-UA*` HTTP request
  ///headers and the `navigator.userAgentData` JS surface on Android. Setting
  ///only [userAgent] does NOT suppress these — Chromium still emits the real
  ///engine + OS values. When `null`, only the low-entropy UA-CH headers
  ///derived from the system default user agent are emitted.
  ///
  ///**Officially Supported Platforms/Implementations**:
  ...
  static InAppWebViewSettings? fromMap(
    ...
    final instance = InAppWebViewSettings(
      ...
      requestedWithHeaderOriginAllowList:
          ...
          : null,
      // [WebSpace fork patch] UA-CH metadata fromMap
  ...
  Map<String, dynamic> toMap({EnumMethod? enumMethod}) {
    return {
      ...
      "requestedWithHeaderOriginAllowList": requestedWithHeaderOriginAllowList
          ?.toList(),
      // [WebSpace fork patch] UA-CH metadata toMap
      "userAgentMetadata": userAgentMetadata?.toMap(),
...
enum InAppWebViewSettingsProperty {
  ...
  // [WebSpace fork patch] UA-CH metadata property gate.
...
extension _InAppWebViewSettingsPropertySupported on InAppWebViewSettings {
  static bool isPropertySupported(
    ...
    switch (property) {
      ...
      // [WebSpace fork patch] UA-CH metadata is Android-only at the native layer.
