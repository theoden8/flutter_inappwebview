...
- Added `proxySettings` property to `InAppWebViewSettings` for per-WebView proxy (iOS 17+ / macOS 14+ `WKWebsiteDataStore.proxyConfigurations`, Linux WPE WebKit 2.40+ `webkit_network_session_set_proxy_settings`). Scoped to a storage session rather than to a WebView, so it is per-WebView only where `containerId` gives that WebView its own session
