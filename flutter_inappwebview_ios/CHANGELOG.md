...
- Fixed a cold-start `EXC_BAD_ACCESS` in `WTF::RunLoop::dispatch` when the first WebKit touch in the process was a `ContainerController` call. `fetchAllDataStoreIdentifiers` and `WKWebsiteDataStore(forIdentifier:)` post their completions to `WTF::RunLoop::main()`, which only exists once WebKit has been initialized on the main thread; on a cold start the background completion could dispatch before init published and crash inside `os_unfair_lock_lock` at address `0x40`. `ContainerManager` now warms WebKit by touching the default data store on the main thread before any identifier-based call
- Implemented per-WebView persistent profile partitioning via `InAppWebViewSettings.containerId` (iOS 17+), wiring `WKWebsiteDataStore(forIdentifier:)` in `preWKWebViewConfiguration(settings:)` with a SHA-256-derived stable UUID
- Implemented per-WebView proxy via `InAppWebViewSettings.proxySettings` (iOS 17+) attached to the WebView's data store, so a profile-bound WebView genuinely uses its own proxy
- `ProxyController.setProxyOverride` / `clearProxyOverride` now reach every container `WKWebsiteDataStore` as well as the default and non-persistent ones, including containers joined after the override was set, so a container-bound WebView can't silently bypass a process-wide proxy. A store a WebView pinned with its own `InAppWebViewSettings.proxySettings` is skipped: the process-wide override is the fallback for sites that named no proxy, so it must not swap the site's proxy for the global one, nor drop the site to the device IP when it is cleared. A rule set that does not fully convert is refused rather than applied as the subset that parsed: `ProxySettings.toProxyConfigurations()` now returns nil in that case (source-breaking for native callers), because the empty array it used to return is routed by `WKWebsiteDataStore` to `clearProxyConfigData` — one unusable rule would have stripped the proxy off a live session
